...
def select_graph(
    ...
    *,
        ...
        for node_index_value, concept_id in enumerate(node_index):
            ...
            for edge_offset, edge in enumerate(valid_edges):
                ...
                if edge.source == concept_id:
                    node_values[forward_flow] = node_values.get(forward_flow, 0.0) + 1.0
                    node_values[reverse_flow] = node_values.get(reverse_flow, 0.0) - 1.0
            ...
            if node_index_value == root:
                for candidate in range(n_nodes):
                    if candidate != root:
                        node_values[candidate] = node_values.get(candidate, 0.0) - 1.0
                _add_row(rows, lower, upper, node_values, 0.0, 0.0)
            else:
                # The accumulated terms above are outflow-inflow. A selected
                # non-root node must instead have inflow-outflow equal to one.
                node_values = {
                    column: -coefficient for column, coefficient in node_values.items()
                }
                node_values[node_index_value] = node_values.get(node_index_value, 0.0) - 1.0
                _add_row(rows, lower, upper, node_values, 0.0, 0.0)
    ...
    try:
        ...
        raise GraphOptimizationError("The MILP solver failed while selecting the graph") from exc
